...
def add_role_to_admin_user(business_entity: BusinessEntity):
    ...
    user_admin = user_repository.get_admin_user_by_business_id(business_entity.props.parent_business_id)

    if user_admin:
        logger.info(f"Adding role to super_admin user {user_admin.id} for business {business_entity.id}")
        user_admin.props.roles.append(BusinessRole(business_id=business_entity.id, role=Role.BUSINESS_ADMIN.value))
