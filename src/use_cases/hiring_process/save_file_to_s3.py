...
def save_file_to_s3_use_case(body: dict, content_type: str, headers: dict) -> tuple:
    ...
    form = cgi.FieldStorage(fp=body_io, environ=environ, headers=headers, keep_blank_values=True)
    ...
    try:
        ...
        s3_client.put_object(
            Bucket=bucket_name, Key=file_key, Body=file_content, ContentType=file_type
        )
