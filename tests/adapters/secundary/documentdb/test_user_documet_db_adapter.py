...
def mock_db_client(mocker):
    mock_client = mocker.patch("src.adapters.secondary.documentdb.user_db_adapter.create_documentdb_client")
    return mock_client
...
def test_get_by_id(mocker, mock_db_client):
    ...
    mock_collection = MagicMock()
    mock_db_client.return_value.__getitem__.return_value = mock_collection
    mock_collection.find_one.return_value = {"_id": "1", "full_name": "John Doe", "email": "john.doe@example.com"}
